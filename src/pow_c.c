...
void *pworkThread(void *pitem)
{
    ...
    pworkInfo->ret = pwork(pworkInfo->mid, pworkInfo->mwm, pworkInfo->nonce, pworkInfo->n);

    pthread_mutex_lock(&mutex);
    if (pworkInfo->ret >= 0) {
        stopC = 1;
        countC += pworkInfo->ret;
        /* This means this thread got the result */
        pworkInfo->n = -1;
    } else {
        countC += 1 - pworkInfo->ret;
    }
...
Trytes *PowC(Trytes *trytes, int mwm)
{
    ...
    Trits *nonce = NULL;
    init_Trits(&nonce);
    nonce->toTrits(nonce, nonce_array[completedIndex], NonceTrinarySize);
