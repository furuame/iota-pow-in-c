...
{
    int i;
    __m256d carry = _mm256_set_pd(LBITS, LBITS,LBITS,LBITS);
...
{
    for (int j = 0; j < n; j++){
        __m256d carry =_mm256_set_pd(HBITS, HBITS, HBITS, HBITS);
        
        for (int i = HASH_LENGTH * 2 / 3; i < HASH_LENGTH && carry[0]; i++) {
            __m256d low = mid_low[i], high = mid_high[i];
            mid_low[i] = _mm256_xor_pd(high, low);
            mid_high[i] = low;
            carry = _mm256_andnot_pd(low,high); // high & (~low);
...
long long int countAVX;

void *pworkThread(void *pitem)
{
    Pwork_struct *pworkInfo = (Pwork_struct *) pitem;

    pworkInfo->ret = pwork256(pworkInfo->mid, pworkInfo->mwm, pworkInfo->nonce, pworkInfo->n);

    pthread_mutex_lock(&mutex);
    if (pworkInfo->ret >= 0) {
        stopAVX = 1;
        countAVX += pworkInfo->ret;
        /* This means this thread got the result */
        pworkInfo->n = -1;
    } else {
        countAVX += 1 - pworkInfo->ret;
    }
