...
__constant__ size_t INDEX[STATE_LENGTH + 1] = {
...
void Launch_init_kernel(CUContext *ctx, size_t block_dim, size_t grid_dim){
  ...
  printf("init kernel <<<%d, %d>>> launching...\n", grid_dim, block_dim);
  init<<<grid, block>>>(
    ctx->trit_hash,
    ctx->mid_low,
    ctx->mid_high,
...
void Launch_search_kernel(CUContext *ctx, size_t block_dim, size_t grid_dim){
  dim3 grid = dim3(grid_dim);
  dim3 block= dim3(block_dim);
  printf("search kernel <<<%d, %d>>> launching...\n", grid_dim, block_dim);
  search<<<grid, block>>>(
    ctx->trit_hash,
    ctx->mid_low,
    ctx->mid_high,
...
void Launch_finalize_kernel(CUContext *ctx, size_t block_dim, size_t grid_dim){
  dim3 grid = dim3(grid_dim);
  dim3 block= dim3(block_dim);
  printf("finalize kernel <<<%d, %d>>> launching...\n", grid_dim, block_dim);
  finalize<<<grid, block>>>(
    ctx->trit_hash,
    ctx->mid_low,
    ctx->mid_high,
...
__device__ void setup_ids(int* id,
                          int* gid,
                          int* gr_id,
                          int* l_size,
                          int* n_trits)
...
__device__ void increment(bc_trit_t* mid_low,
                          bc_trit_t* mid_high,
                          size_t from_index,
                          size_t to_index) 
{
...
__device__ void copy_mid_to_state(bc_trit_t* mid_low,
                                  bc_trit_t* mid_high,
                                  bc_trit_t* state_low,
                                  bc_trit_t* state_high,
                                  size_t id,
...
__device__  void transform(bc_trit_t* state_low,
                           bc_trit_t* state_high,
                           size_t id,
                           size_t l_size,
                           size_t n_trits) 
{
  size_t round, i, j, k;
  bc_trit_t alpha, beta, gamma, delta, sp_low[3], sp_high[3];
...
__device__  void check(bc_trit_t* state_low,
                       bc_trit_t* state_high,
                       volatile char* found,
                       size_t* min_weight_magnitude,
                       bc_trit_t* nonce_probe,
...
__global__ void init( char* trit_hash,
                      bc_trit_t* mid_low,
                      bc_trit_t* mid_high,
                      bc_trit_t* state_low,
                      bc_trit_t* state_high,
                      ...
                      size_t* loop_count) 
{
  int i, j, id, gid, gr_id, gl_off, l_size, n_trits;
  ...
  /*if(threadIdx.x==0){
    printf("kernel init launched, gid:%d gr_id:%d l_size:%d n_trits:%d\n", gid, gr_id, l_size, n_trits);
  }*/
}

__global__ void search(char* trit_hash,
                      bc_trit_t* mid_low,
                      bc_trit_t* mid_high,
                      bc_trit_t* state_low,
                      bc_trit_t* state_high,
                      ...
                      size_t* loop_count) 
{
  int i, id, gid, gr_id, l_size, n_trits;
  setup_ids(&id, &gid, &gr_id, &l_size, &n_trits);
...
__global__ void finalize(char* trit_hash,
                      bc_trit_t* mid_low,
                      bc_trit_t* mid_high,
                      bc_trit_t* state_low,
                      bc_trit_t* state_high,
                      ...
                      size_t* loop_count)
{
  int i, j, id, gid, gr_id, l_size, n_trits;
  setup_ids(&id, &gid, &gr_id, &l_size, &n_trits);
