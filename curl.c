...
static void Transform(Curl *thiz)
{
    _transform(thiz->state->data);
}
...
static Trytes *Squeeze(Curl *thiz)
{
    ...
    */
    ...
    thiz->Transform(thiz);
    return ret->toTrytes(ret);
}

static void Absorb(Curl *thiz, Trytes *inn)
{
    Trits *in = inn->toTrits(inn);
    int lenn = 0;
...
Curl *NewCurl()
{
    ...
    Curl *ret = (Curl *) malloc(sizeof(Curl));
    char trt[stateSize] = {0};
    init_Trits(&ret->state);
    ret->state->toTrits(ret->state, trt, stateSize);
    ret->Squeeze = Squeeze;
    ret->Absorb = Absorb;
