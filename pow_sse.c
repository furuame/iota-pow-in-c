...
void *pworkThread(void *pitem)
{
    ...
    pworkInfo->ret = pwork128(pworkInfo->mid, pworkInfo->mwm, pworkInfo->nonce, pworkInfo->n);

    pthread_mutex_lock(&mutex);
    if (pworkInfo->ret >= 0) {
        stopSSE = 1;
        countSSE += pworkInfo->ret;
        /* This means this thread got the result */
        pworkInfo->n = -1;
    } else {
        countSSE += 1 - pworkInfo->ret;
    }
    pthread_mutex_unlock(&mutex);
    pthread_exit(NULL);
}

Trytes *PowSSE(Trytes *trytes, int mwm)
{
    stopSSE = 0;
    countSSE = 0;
